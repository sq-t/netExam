...
//点击查询图片，实现查询
function search() {
	location.href = "manager_lesson_message.servlet?condition=" + document.getElementById("condition").value;
}
...
function jmp_page() {
	...
	if(jmp_page <= totalPages && jmp_page > 0) {
		location.href = "manager_lesson_message.servlet?page=" + jmp_page + "&totalUsers=" + totalUsers;
	}
	else {
...
function delete_lesson_js() {
	layer.confirm('确认删除吗？', {
		  btn: ['确认', '取消'] //可以无限个按钮
			...
			location.href = "delete_lesson?delete_arr=" + arr;
		}, function(index){
