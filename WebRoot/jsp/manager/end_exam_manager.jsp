...
<head>
	<meta charset="UTF-8">
	<title>在线考试星已结束考试</title>
	<!-- base需要放到head中,这里我们就可以直接使用相对路径(即: 相对于base标签) -->
	<base href="<%=basePath%>">
	<link href="css/menu.css" rel="stylesheet" type="text/css">
	<link href="css/table_end_exam_manager.css" rel="stylesheet" type="text/css">
	
	<script src="js/end_exam_manager.js" type="text/javascript"></script>
...
<div class="tool-wrap">
	<div class="tool-content-wrap">
    	<div class="tool-content">
    		...
    		<a>
        		<button id="RestartExam"  class="delete-button" disabled="disabled" onclick="restart_exam_js()">
            		重启考试
        		</button>
    		</a>
		</div>
...
<table>
		...
		<c:if test="${totalUsers != 0}">
			<c:forEach var="exam" items="${currentPageUsers}">
	                <tr>
	                    ...
	                    <td>${exam.duration}</td>
	                    <td style="width: 30px">
	                        <a title="重启考试" onclick="restart_exam(${exam.id})" >
	                            <img src="img/重启.png">
	                        </a>
	                    </td>
	                </tr>
	...
	<div class="text-center">
		<nav>
			<ul class="pagination">
				<li>
					<a href="<c:url value="/manager_exam_message.servlet?status=考试已结束&to_page=end_exam_manager.jsp&page=1&totalUsers=${totalUsers}"/>">首页</a>
				</li>
				<li>
					<a href="<c:url value="/manager_exam_message.servlet?status=考试已结束&to_page=end_exam_manager.jsp&page=${page-1>1?page-1:1}&totalUsers=${totalUsers}"/>">&laquo;</a>
				</li>
				<c:forEach begin="1" end="${totalPages}" varStatus="loop">
					<c:set var="active" value="${loop.index==page?'active':''}" />
					<li class="${active}">
						<a href="<c:url value="/manager_exam_message.servlet?status=考试已结束&to_page=end_exam_manager.jsp&page=${loop.index}&totalUsers=${totalUsers}"/>">${loop.index}</a>
					</li>
				</c:forEach>
				<li>
					<a href="<c:url value="/manager_exam_message.servlet?status=考试已结束&to_page=end_exam_manager.jsp&page=${page+1<totalPages?page+1:totalPages}&totalUsers=${totalUsers}"/>">&raquo;</a>
				</li>
				<li>
					<a href="<c:url value="/manager_exam_message.servlet?status=考试已结束&to_page=end_exam_manager.jsp&page=${totalPages}&totalUsers=${totalUsers}"/>">尾页&nbsp;&nbsp;&nbsp;&nbsp;</a>
				</li>
				<li class="jmp">
